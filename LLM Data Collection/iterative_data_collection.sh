...
client_iperf3_script() {
    ...
    ssh -p "$src1port" -i "$sshkeypath" root@"$vmhostaddr" "iperf3 -c 172.16.1.2 -t $duration -p 5101 -J > iperf3_client_${tcp1}_${iter}_$aqm.json" &
    ssh -p "$src2port" -i "$sshkeypath" root@"$vmhostaddr" "iperf3 -c 172.16.1.2 -t $duration -p 5102 -J > iperf3_client_${tcp2}_${iter}_$aqm.json"
    sleep $end_wait_time
}
