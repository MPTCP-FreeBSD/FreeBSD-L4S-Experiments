...
ssh -p $router1port -i ~/.ssh/$sshkey root@$vmhostaddr "ipfw pipe 1 config bw $vm1drate delay $vm1delay codel noecn"
...
ssh -p $router2port -i ~/.ssh/$sshkey root@$vmhostaddr "ipfw pipe 1 config bw $vm2drate delay $vm1delay codel noecn"
ssh -p $router1port -i ~/.ssh/$sshkey root@$vmhostaddr "ipfw add 100 pipe 1 ip from any to any"
