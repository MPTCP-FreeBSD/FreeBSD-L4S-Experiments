...
ssh -p $router1port -i ~/.ssh/$sshkey root@$vmhostaddr "sysctl net.inet.tcp.ecn.enable=2"
ssh -p $router1port -i ~/.ssh/$sshkey root@$vmhostaddr "ipfw pipe 1 config bw $vm1drate delay $vm1delay"
ssh -p $router1port -i ~/.ssh/$sshkey root@$vmhostaddr "ipfw sched 1 config pipe 1 type fq_pie max_burst 100 noecn"
...
ssh -p $router2port -i ~/.ssh/$sshkey root@$vmhostaddr "ipfw sched 1 config pipe 1 type fq_pie max_burst 100 noecn"
